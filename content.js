...
(function () {
    ...
    // Global try-catch to handle extension context invalidation
    try {
        // Guard: Check if chrome APIs are available (prevents errors when extension is reloaded)
        ...
        // ====== DEBOUNCED AUTOSAVE ======
        // Collects results and saves once after extraction stabilizes
        let pendingAutosave = null;
        let autosaveTimeout = null;

        function queueAutosave(result) {
            // Merge with pending results
            if (!pendingAutosave) {
                pendingAutosave = {
                    emails: [],
                    phones: [],
            ...
            if (result.emails) pendingAutosave.emails.push(...result.emails);
            if (result.phones) pendingAutosave.phones.push(...result.phones);
            if (result.socialLinks) pendingAutosave.socialLinks.push(...result.socialLinks);
            if (result.serpLinks?.linkedin) pendingAutosave.serpLinks.linkedin.push(...result.serpLinks.linkedin);
        ...
        function autoExtractOnLoad() {
            ...
            safeStorageGet(['settings'], (data) => {
                // Handle case where settings might not exist yet
                const settings = data.settings || {};

                // Get active state from settings.active (this is where popup stores it)
                // Default to TRUE if not set (extension should be active by default)
                ...
                if (!isActive) {
                    ...
                    safeSendMessage({ type: 'UPDATE_BADGE', count: 0 });
                    return;
                }

                // Get extraction settings from settings object (default all ON)
                const extractEmails = settings.extractEmails !== false;
                const extractPhones = settings.extractPhones !== false;
                const extractSocials = settings.extractSocials !== false;
                const extractSerpLinks = settings.extractSerp === true;
                const selectedCountries = settings.selectedCountries || [];
                ...
                try {
                    const result = safeExtractData(request);
                    const totalLeads = (result.emails?.length || 0) + (result.phones?.length || 0) + (result.socialLinks?.length || 0);

                    // AUTOSAVE: If autosave is enabled AND we have data, queue for saving
                    if (autosave && totalLeads > 0) {
                        queueAutosave(result);
                    }
                } catch (e) {
                    // Extraction error - ignore
                }
            });
        }
        ...
        function autosaveToStorage(results) {
            ...
            safeStorageGet(['saved'], (data) => {
                try {
                    ...
                    // Save to storage
                    safeStorageSet({ saved: mergedSaved });
                } catch (e) {
                    // Autosave error - ignore
                }
            });
        }

        // Run auto-extraction FAST with multiple attempts for reliability
        // First attempt: 100ms (for fast pages)
        setTimeout(() => {
            autoExtractOnLoad();
        }, 100);

        // Second attempt: 500ms (for medium pages)
        setTimeout(() => {
            autoExtractOnLoad();
        }, 500);

        // Third attempt: 1500ms (for slow pages with dynamic content)
        setTimeout(() => {
            autoExtractOnLoad();
        }, 1500);
        ...
        const observer = new MutationObserver(() => {
            // Debounce: only re-extract if 1+ second since last extraction
            if (Date.now() - lastExtractTime > 1000) {
                lastExtractTime = Date.now();
                autoExtractOnLoad();
            }
        });

        // Observe body for major changes - start quickly
        setTimeout(() => {
            if (document.body) {
        ...
        // ====== URL CHANGE DETECTION (For SPA sites like Google Search) ======
        // This catches navigation between search result pages
        ...
        setInterval(() => {
            ...
            if (currentUrl !== lastUrl) {
                console.log('[Email Extractor Pro] URL changed, re-extracting...');
                lastUrl = currentUrl;
                lastExtractTime = Date.now();

                // Extract after short delay for new content to load
                setTimeout(() => autoExtractOnLoad(), 100);
                setTimeout(() => autoExtractOnLoad(), 500);
        ...
        window.addEventListener('popstate', () => {
            console.log('[Email Extractor Pro] Navigation detected, re-extracting...');
            lastExtractTime = Date.now();
            setTimeout(() => autoExtractOnLoad(), 100);
            setTimeout(() => autoExtractOnLoad(), 500);
        });

        // Listen for click on pagination links (Google-specific)
        document.addEventListener('click', (e) => {
            const target = e.target.closest('a');
            if (target && (target.href?.includes('start=') || target.id?.includes('pn'))) {
                console.log('[Email Extractor Pro] Pagination click detected');
                setTimeout(() => autoExtractOnLoad(), 500);
                setTimeout(() => autoExtractOnLoad(), 1000);
                setTimeout(() => autoExtractOnLoad(), 1500);
            }
        }, true);

        console.log('[Email Extractor Pro] Content script loaded (v3.5 - Error Handled)');

    } catch (e) {
        // Silently handle extension context invalidation errors
        // This happens when extension is reloaded while content scripts are running
    }
})();
